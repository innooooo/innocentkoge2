...
function coverflowimages(options) {
	if (!document.getElementById("coverflowoverlay")) {
		...
		var $enlargearea = $('<div id="coverenlargearea" />').appendTo(
			document.body
		);
	} else {
	...
	function selecteditemaction(e, activeitem) {
		// default call back function for onselecteditemclick, showing enlarged image
		$overlay.css({ opacity: 0.9, zIndex: 1000 });
		$enlargearea.html(
			'<img src="' +
				options.images[activeitem][1] +
				'"/>' +
				(options.images[activeitem][2]
					? '<div id="desc">' + options.images[activeitem][2] + "</div>"
					: "")
		);

		var maxheight = $(window).height();
		$enlargearea
			.css({ opacity: 1, zIndex: 1001 })
			.data("isvisible", true)
			.find("img:eq(0)")
			.css({
	...
	$frame.on("mousedown", function (e) {
		// test during 'mousedown' phase if distination LI is active item, before 'onclick'
	...
	$frame.on("click", function (e) {
		// onclick the main cover flow container
		var $target = $(e.target);
